    ...
    return cryptos.filter(c => c.isListed).sort((a, b) => {
      ...
      return (
        <Col key={index} className="masternodesListingCoinInfoContainer">
          <Col onClick={() => this.onSelectMasternode(crypto.slug)} xl={{size: 2, offset: 0}} lg={{size: 2, offset: 0}} md={{size: 2, offset: 0}} sm={{size: 6, offset: 3}} className="masternodeNameSectionContainer d-flex flex-xl-column flex-lg-column flex-md-column flex-row align-items-center justify-content-center px-0">
