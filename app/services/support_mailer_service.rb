class SupportMailerService
  ...
  def self.send_auto_withdrawal_notification(user, reward)
    SupportMailer.send_email("User #{user.email} received a reward with auto withdrawal turned on.", "User #{user.email} received new reward of
      #{reward.total_amount} #{reward.symbol} for auto withdrawal.").deliver_later
  end
